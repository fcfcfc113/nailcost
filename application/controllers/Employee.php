...
class Employee extends CI_Controller
{
    ...
    public function quotes_list()
    {
        ...
        $eid = $this->input->post('eid');
        $list = $this->employee->quotes_datatables($eid);
        $data = array();
        ...
        foreach ($list as $quotes) {
            ...
            $row[] = $quotes->tid;
            $row[] = $quotes->name;
            $row[] = $quotes->quotedate;
            $row[] = amountExchange($quotes->total, 0, $this->aauth->get_user()->loc);
            ...
            $row[] = $out;
            $row[] = '<a href="' . base_url("quote/view?id=$quote->id") . '" class="btn btn-success btn-xs"><i class="fa fa-eye"></i> View</a> &nbsp; <a href="' . base_url("quote/printiquote?id=$quotes->id") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->employee->quotescount_all($eid),
            "recordsFiltered" => $this->employee->quotescount_filtered($eid),
            "data" => $data,
        );
        //output to json format
        echo json_encode($output);
